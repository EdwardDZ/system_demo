layui.config({
	...
	var form = layui.form,table = layui.table;
		...
		table.render({
			id:'roleList',
		    elem: '#roleList'
		    ,url: basePath+'/sys/getRoleList' //数据接口
		    ,cellMinWidth: 80
		    ,limit:10//每页默认数
		    ,limits:[10,20,30,40]
		    ,cols: [[ //表头
		  ...
		  table.on('tool(roleList)', function(obj){
		    ...
		    if(obj.event === 'del'){
		      ...
		      layer.confirm('真的删除行么', function(index){
		    	  $.ajax({
		    		  url:basePath+'/sys/delRole/'+data.roleId,
		    		  type : "get",
		    		  success : function(d){
		    			  if(d.code==0){
		    				  //obj.del();
		      ...
		      layer.open({
		    	  ...
		    	  title:"编辑角色",
		    	  area: ['380px', '600px'],
		    	  content:basePath+"/sys/editRole?roleId="+data.roleId+"&roleName="+data.roleName+"&roleRemark="+data.roleRemark, //这里content是一个普通的String
		      })
		    }
		  });
		...
		if(data.length>0){
			...
			//包含不允许操作角色，结束方法
			  ...
			  layer.confirm('真的要删除<strong>'+data.length+'</strong>条数据吗？', function(index){
				//调用删除接口
				  $.ajax({
			    		  url:basePath+'/sys/delRoles/'+roleStr,//接口地址
			    		  type : "get",
			    		  success : function(d){
			    			  if(d.code==0){
			    				  //删除成功，刷新父页面
