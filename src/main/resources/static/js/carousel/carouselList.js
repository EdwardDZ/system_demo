layui.config({
	...
	var form = layui.form,table = layui.table;
		...
		table.render({
			id:'carouselList',
		    elem: '#carouselList'
		    ,url: basePath+'/carousel/list' //数据接口
		    ,cellMinWidth: 80
		    ,limit:10//每页默认数
		    ,limits:[10,20,30,40]
		    ,cols: [[ //表头
              ...
              ,{field:'imgurl', title: '轮播图',templet:'<div><img src="'+basePath+'/file/showPic?fileName={{ d.imgurl}}"></div>'}
		  ...
		  table.on('tool(test)', function(obj){
		    ...
		    if(obj.event === 'del'){
		      layer.confirm('真的删除行么?', function(index){
		    	  $.ajax({
		    		  url:basePath+'/carousel/delete?id='+data.id,
		    		  type : "get",
		    		  success : function(d){
		    			  if(d.code==0){
		      ...
		      layer.open({
		    	  ...
		    	  title:"编辑轮播图",
		    	  area: ['380px', '460px'],
		    	  content:basePath+"/carousel/editCarousel?id="+data.id //这里content是一个普通的String
		      })
		    }
		  });
	...
	$(".carouselAdd_btn").click(function(){
		layer.open({
	    	  ...
	    	  title:"添加轮播",
	    	  area: ['380px', '460px'],
	    	  content:basePath+"/carousel/addCarousel", //这里content是一个普通的String
	      })
	})
